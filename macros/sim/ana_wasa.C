            ...
            */
            for (Int_t j = 0; j < nHits; j++)
            {
                ...
                if (ID == -1)
                {
                    ...
                    {
                        Data[j]->Momentum(mom);
                        if (Data[j]->GetPid() == 211 && mom.Mag() > 0.05)
                            fh2_P_ang->Fill(momt.Theta() * TMath::RadToDeg() + gRandom->Gaus(0., 0.5),
                                            mom.Mag() + gRandom->Gaus(0., mom.Mag() * emom));
                        // cout <<Data[j]->GetPid() <<" "<< mom.Mag()<<" "<< TrackData[track]->GetPdgCode() << endl;
                        if (Data[j]->GetPid() == 2212 && mom.Mag() > 0.233)
                            fh2_P_Energy->Fill(
                                mom.Mag(),
                                Data[j]->GetEnergyLoss() + gRandom->Gaus(0., Data[j]->GetEnergyLoss() * eelos));
                        if (Data[j]->GetPid() == 211 && mom.Mag() > 0.05)
                            fh2_P_Energy->Fill(
                                mom.Mag(),
                                Data[j]->GetEnergyLoss() + gRandom->Gaus(0., Data[j]->GetEnergyLoss() * eelos));
