...
FrsSciRawTofPar::FrsSciRawTofPar(const char *name, const char *title,
                                 const char *context)
    : FairParGenericSet(name, title, context), fFirstStaSci(1),
      fNumSignals(1) //(NUMBER_OF_SOFSCI_DETECTORS - 1) with
                     //NUMBER_OF_SOFSCI_DETECTORS=2
      ,
      fNumParsPerSignal(2) {
...
void FrsSciRawTofPar::printParams() {
  ...
  for (Int_t param = 0; param < array_size; param++) {
    cout << "LIMIT " << param << " = " << fAllSignalsRawTofParams->GetAt(param)
         << endl;
  }
}
