...
public:
  ...
  Double_t GetSignalTcalParams(UInt_t rank) {
    return (Double_t)fAllSignalsRawPosParams->GetAt(rank);
  }
  ...
  void SetSignalParams(Double_t parval, UInt_t rank) {
    fAllSignalsRawPosParams->AddAt(parval, rank);
  }

  /** Create more Methods if you need them! **/

private:
  TArrayF *fAllSignalsRawPosParams; // Calibration Parameters for all signals of
                                    // one detector
  Int_t fNumChannels;               // number of detectors
  Int_t fNumDetectors; // number of channels of SciTcal data = 3 (need for
                       // Tcal2SingleTcal)
  Int_t fNumSignals;   // =fNumDetectors if RawPos
  Int_t fNumParsPerSignal;
  const FrsSciRawPosPar &
  operator=(const FrsSciRawPosPar &); /*< an assignment operator>*/
