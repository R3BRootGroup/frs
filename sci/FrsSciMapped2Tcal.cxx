...
FrsSciMapped2Tcal::FrsSciMapped2Tcal()
    : FairTask("FrsSciMapped2Tcal", 1), fNumTcal(0), fNevent(0), fTcal(NULL),
      fMapped(NULL), fTcalPar(NULL), fOnline(kFALSE) {}

// --- Standard Constructor
FrsSciMapped2Tcal::FrsSciMapped2Tcal(const char *name, Int_t iVerbose)
    : FairTask(name, iVerbose), fNumTcal(0), fNevent(0), fTcal(NULL),
      fMapped(NULL), fTcalPar(NULL), fOnline(kFALSE) {}
...
InitStatus FrsSciMapped2Tcal::Init() {
  ...
  if (fTcalPar->GetNumSignals() == 0) {
    ...
    LOG(INFO) << " FrsSciMapped2Tcal::Init(): fNumDetectors="
              << fTcalPar->GetNumDetectors();
    LOG(INFO) << "  FrsSciMapped2Tcal::Init(): fNumChannels="
              << fTcalPar->GetNumChannels();
  }
  return kSUCCESS;
}
...
void FrsSciMapped2Tcal::Exec(Option_t *option) {
  ...
  for (Int_t ihit = 0; ihit < nHitsPerEvent_SofSci; ihit++) {
    ...
    if ((iDet < 1) || (iDet > fTcalPar->GetNumDetectors())) {
      LOG(INFO) << "FrsSciMapped2Tcal::Exec() : In SofSciMappedData, iDet = "
                << iDet << "is out of range, item skipped ";
      continue;
    }
    if ((iCh < 1) || (iCh > fTcalPar->GetNumChannels())) {
      LOG(INFO) << "FrsSciMapped2Tcal::Exec() : In SofSciMappedData, iCh = "
                << iCh << "is out of range, item skipped ";
      continue;
    }
...
// --------------------------------------------
FrsSciTcalData *FrsSciMapped2Tcal::AddCalData(Int_t iDet, Int_t iCh,
                                              Double_t tns) {
...
Double_t FrsSciMapped2Tcal::CalculateTimeNs(UShort_t iDet, UShort_t iCh,
                                            UInt_t iTf, UInt_t iTc) {
  UInt_t rank = iTf + fTcalPar->GetNumTcalParsPerSignal() *
                          ((iDet - 1) * fTcalPar->GetNumChannels() + (iCh - 1));
  Double_t iPar = (Double_t)fTcalPar->GetSignalTcalParams(rank);
  Double_t r = (Double_t)rand.Rndm() - 0.5;
  Double_t iTf_ns;
  Double_t iTc_ns = (Double_t)iTc * 5.;
  //  std::cout << "FrsSciMapped2Tcal::CalculateTimeNs : iDet=" << iDet << ",
  //  iCh=" << iCh << ", iTf=" << iTf << ", rank=" << rank  << std::endl;
  ...
  // std::cout << "Tf_ns=" << iTf_ns << ", iTc_ns=" << iTc_ns << " : TimeNs = "
  // << 5.*iTc_ns - iTf_ns << std::endl;
