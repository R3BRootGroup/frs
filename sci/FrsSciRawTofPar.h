...
class FrsSciRawTofPar : public FairParGenericSet
{
  ...
  private:
    TArrayF* fAllSignalsRawTofParams; // Calibration Parameters for all signals of
                                      // one detector
    Int_t fFirstStaSci;               // if nDets>2, start the mult selection with detectors
                                      // number fFirstStaSci (1-based)
    Int_t fNumSignals;                // = number of Tof spectra with mult=1 =
                                      // NUMBER_OF_SCI_DETECTORs-1
