...
FrsSciTcal2RawTofPar::FrsSciTcal2RawTofPar()
    : FairTask("FrsSciTcal2RawTofPar", 1), fNumDetectors(2), fNumChannels(2),
      fFirstStaSci(1), fNumSignals(1), fNumParsPerSignal(2),
      fMinStatistics(100), fTcal(NULL), fRawTofPar(NULL) {}

// FrsSciTcal2RawTofPar: Standard Constructor --------------------------
FrsSciTcal2RawTofPar::FrsSciTcal2RawTofPar(const char *name, Int_t iVerbose)
    : FairTask(name, iVerbose), fNumDetectors(2), fNumChannels(2),
      fFirstStaSci(1), fNumSignals(1), fNumParsPerSignal(2),
      fMinStatistics(100), fTcal(NULL), fRawTofPar(NULL) {}
...
InitStatus FrsSciTcal2RawTofPar::Init() {
  ...
  if (!fTcal) {
    LOG(error) << "FrsSciTcal2RawTofPar::Couldn't get handle on FrsSciTcalData "
                  "container";
    return kfatal;
  }
  ...
  if (!fRawTofPar) {
    LOG(error) << "FrsSciTcal2RawTofPar::Couldn't get handle on "
                  "FrsSciRawTofPar container";
    return kfatal;
  }
  ...
  for (Int_t detstart = 0; detstart < 1; detstart++) {
    sprintf(name, "TofRaw_Sci%i_to_Sci%i", 20 + detstart + 1,
            40 + detstart + 1);
...
void FrsSciTcal2RawTofPar::Exec(Option_t *opt) {
  ...
  UInt_t nHitsSci =
      fTcal->GetEntries(); // can be very high especially for S2 detector
  ...
  for (UInt_t ihit = 0; ihit < nHitsSci; ihit++) {
    ...
    if (!hitSci) {
      LOG(warn) << "FrsSciTcal2RawTofPar::Exec() : could not get hitSci";
  ...
  for (UShort_t dstart = 0; dstart < fNumDetectors - 1; dstart++) {
    ...
    if ((mult[dstart * fNumChannels] == 1) &&
        (mult[dstart * fNumChannels + 1] == 1) &&
        (mult[dstop * fNumChannels] == 1) &&
        (mult[dstop * fNumChannels + 1] == 1)) {
...
void FrsSciTcal2RawTofPar::CalculateRawTofParams() {
  ...
  for (Int_t sig = 0; sig < fNumSignals; sig++) {
    ...
    if (fh_RawTofMult1[sig]->GetEntries() > fMinStatistics && iMax > 0) {
      ...
      fRawTofPar->SetSignalParams(fh_RawTofMult1[sig]->GetBinLowEdge(binLimit),
                                  sig * 2);
      // HIGHER LIMIT
