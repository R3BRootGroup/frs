...
public:
  ...
  void SetNumChannels(Int_t NumberOfChannels) {
    fNumChannels = NumberOfChannels;
  }
...
private:
  TArrayF *fAllSignalsTcalParams; // Calibration Parameters for all signals of
                                  // one detector
  ...
  const FrsSciTcalPar &
  operator=(const FrsSciTcalPar &); /*< an assignment operator>*/
