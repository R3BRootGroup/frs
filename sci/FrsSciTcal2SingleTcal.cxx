...
FrsSciTcal2SingleTcal::FrsSciTcal2SingleTcal()
    : FairTask("FrsSciTcal2SingleTcal", 1), fTcal(NULL), fRawPosPar(NULL),
      fRawTofPar(NULL), fSingleTcal(NULL), fNumSingleTcal(0), fOnline(kFALSE),
      fNevent(0) {}
...
void FrsSciTcal2SingleTcal::Exec(Option_t *option) {
  ...
  for (int ihit = 0; ihit < nHitsPerEvent_SofSci; ihit++) {
    ...
    iCh = hit->GetPmt() - 1;
    if (mult_max > 32)
      continue; // if multiplicity in a Pmt is higher than 16 are discarded,
                // this code cannot handle it
  ...
  // It makes no sense to continue if there is no Left and Right signal on the
  // SofSci at cave C
  if ((nHitsPerEvent_SofSci > 0) && (mult[(ID_SOFSCI - 1) * nChs] > 0) &&
    ...
    // SELECTION OF THE MULTIPLICITY LOOKING AT THE ToFraw VERSUS Cave C
    // SCINTILLATOR Since the multiplicity at S2 is much lower than at S4 all
    // the hit finder are done looking at the Tof from S2 to S4 with conditions
    // on Pos and ToF
    ...
    for (UShort_t multRsta = 0; multRsta < mult[dSta * nChs]; multRsta++) {
      for (UShort_t multLsta = 0; multLsta < mult[dSta * nChs + 1];
        ...
        iRawPos =
            (iTraw[dSta * nChs][multRsta] - iTraw[dSta * nChs + 1][multLsta]);
        ...
        for (UShort_t multRsto = 0; multRsto < mult[dSto * nChs]; multRsto++) {
          for (UShort_t multLsto = 0; multLsto < mult[dSto * nChs + 1];
               multLsto++) {
            iRawPos = (iTraw[dSto * nChs][multRsto] -
                       iTraw[dSto * nChs + 1][multLsto]);
            if ((iRawPos < fRawPosPar->GetSignalTcalParams(2 * dSto)) ||
                (iRawPos > fRawPosPar->GetSignalTcalParams(2 * dSto + 1)))
              continue;
            iRawTime_dSta = 0.5 * (iTraw[dSta * nChs][multRsta] +
                                   iTraw[dSta * nChs + 1][multLsta]);
            iRawTime_dSto = 0.5 * (iTraw[dSto * nChs][multRsto] +
                                   iTraw[dSto * nChs + 1][multLsto]);
            // iRawTof = iRawTime_dSto - iRawTime_dSta + iTraw[dSta * nChs +
            // 2][0] - iTraw[dSto * nChs + 2][0];
    ...
    if (mult_selectHits[dSto] > 0) {
      for (dSta = 0; dSta < nDets - 1; dSta++) {
        ...
        for (UShort_t multRsta = 0; multRsta < mult[dSta * nChs]; multRsta++) {
          for (UShort_t multLsta = 0; multLsta < mult[dSta * nChs + 1];
            ...
            iRawPos = (iTraw[dSta * nChs][multRsta] -
                       iTraw[dSta * nChs + 1][multLsta]);
            if ((fRawPosPar->GetSignalTcalParams(2 * dSta) > iRawPos) ||
                (iRawPos > fRawPosPar->GetSignalTcalParams(2 * dSta + 1)))
              continue;
            iRawTime_dSta = 0.5 * (iTraw[dSta * nChs][multRsta] +
                                   iTraw[dSta * nChs + 1][multLsta]);
            iRawTime_dSto =
                0.5 * (iTraw[dSto * nChs][selectLeftHit[dSto]] +
      ...
      FrsSciSingleTcalData *fItem =
          (FrsSciSingleTcalData *)fSingleTcal->At(fNumSingleTcal - 1);

      for (UShort_t d = 0; d < nDets; d++) {
        if (mult_selectHits[d] > 0) {
          RawPos[d] = iTraw[d * nChs][selectRightHit[d]] -
                      iTraw[d * nChs + 1][selectLeftHit[d]];
          RawTime[d] = 0.5 * (iTraw[d * nChs][selectRightHit[d]] +
                              iTraw[d * nChs + 1][selectLeftHit[d]]);
        } else {
          RawPos[d] = -1000000.;
          RawTime[d] = -1000000.;
      ...
      for (UShort_t dstart = 0; dstart < nDets - 1; dstart++) {
        for (UShort_t dstop = dstart + 1; dstop < nDets; dstop++) {
          if ((mult_selectHits[dstart] > 0) && (mult_selectHits[dstop] > 0)) {
            RawTof[iTof] = RawTime[dstop] - RawTime[dstart];
          } else
            RawTof[iTof] = 1000000.;
    ...
    for (UShort_t d = 0; d < nDets; d++) {
      ...
      for (UShort_t multR = 0; multR < mult[d * nChs]; multR++) {
        for (UShort_t multL = 0; multL < mult[d * nChs + 1]; multL++) {
          iRawPos =
              iTraw[d * nChs][multR] -
              iTraw[d * nChs + 1][multL]; // Raw position = Tright - Tleft for x
          ...
          // if ((((maskR[d] >> multR) & (0x1)) == 1) || (((maskL[d] > multL) &
          // (0x1)) == 1))
          //    continue;
          ...
          RawTime[d] =
              0.5 * (iTraw[d * nChs][multL] + iTraw[d * nChs + 1][multR]);
          // tag which hit is used
          // maskR[d] |= (0x1) << multR;
          // maskL[d] |= (0x1) << multL;
          // implement how many "good event" is found
          // attention for a single detector at S2, this might not be sufficient
          // if the searching window of the VFTX is too large
          mult_selectHits[d]++;
