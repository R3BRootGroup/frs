...
void FrsSciTcal2SingleTcal::Exec(Option_t* option)
{
    ...
    // SofSci at cave C
    if ((nHitsPerEvent_SofSci > 0) && (mult[(ID_SOFSCI - 1) * nChs] > 0) && (mult[(ID_SOFSCI - 1) * nChs + 1] > 0))
    {
        Double_t iRawPos;
        ...
        for (UShort_t d = 0; d < nDets; d++)
        {
            ...
            for (UShort_t multR = 0; multR < mult[d * nChs]; multR++)
            {
                ...
                {
                    iRawPos = iTraw[d * nChs][multR] - iTraw[d * nChs + 1][multL]; // Raw position = Tright - Tleft for
