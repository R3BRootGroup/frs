...
FrsSciTcal2RawPosPar::FrsSciTcal2RawPosPar()
    : FairTask("FrsSciTcal2RawPosPar", 1), fNumDetectors(2), fNumChannels(2),
      fNumParsPerSignal(2), fMinStatistics(100), fTcal(NULL), fRawPosPar(NULL),
      fOutputFile(NULL) {
...
void FrsSciTcal2RawPosPar::CalculateRawPosRawPosParams() {
  ...
  for (Int_t sig = 0; sig < fNumDetectors; sig++) {
    if (fh_RawPosMult1[sig]->GetEntries() > fMinStatistics) {
      ...
      fRawPosPar->SetSignalParams(fh_RawPosMult1[sig]->GetBinLowEdge(binLimit),
                                  sig * 2);
      // HIGHER LIMIT
