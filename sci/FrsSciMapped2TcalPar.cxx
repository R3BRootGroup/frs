...
FrsSciMapped2TcalPar::FrsSciMapped2TcalPar()
    : FairTask("FRSSciMapped2TcalPar", 1), fNumDetectors(2), fNumChannels(2),
      fNumTcalParsPerSignal(1000), fMinStatistics(0), fMapped(NULL),
...
InitStatus FrsSciMapped2TcalPar::Init() {
  ...
  for (Int_t det = 0; det < fNumDetectors; det++) {
    for (Int_t ch = 0; ch < fNumChannels; ch++) {
      sprintf(name, "TimeFineBin_Sci%i_Ch%i_Sig%i", det + 1, ch + 1,
              det * fNumChannels + ch);
      fh_TimeFineBin[det * fNumChannels + ch] =
          new TH1F(name, name, fNumTcalParsPerSignal, 0, fNumTcalParsPerSignal);
      sprintf(name, "TimeFineNs_Sci%i_Ch%i_Sig%i", det + 1, ch + 1,
              det * fNumChannels + ch);
...
void FrsSciMapped2TcalPar::Exec(Option_t *opt) {
  ...
  // for the scintillator this number of hits can be very large especially for
  // the detector at S2
  UInt_t nHitsSci =
      fMapped->GetEntries(); // can be very high especially for S2 detector
  UInt_t iSignalSci;
  ...
  for (Int_t sig = 0; sig < fNumSignals; sig++) {
    if (fh_TimeFineBin[sig]->GetEntries() > fMinStatistics) {
      ...
      for (Int_t bin = 0; bin < fNumTcalParsPerSignal; bin++) {
        ...
        fTcalPar->SetSignalTcalParams(Bin2Ns[bin],
                                      sig * fNumTcalParsPerSignal + bin);
      }
    }
