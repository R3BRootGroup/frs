...
FrsSciTcalPar::FrsSciTcalPar(const char *name, const char *title,
                             const char *context)
    : FairParGenericSet(name, title, context), fNumDetectors(2),
      fNumChannels(2), fNumTcalParsPerSignal(10) {
  ...
  for (Int_t d = 0; d < fNumDetectors; d++) {
    for (Int_t ch = 0; ch < fNumChannels; ch++) {
      ...
      /*
        ...
        {
        cout << "FineTime at Bin (" << bin << ") = " <<
        fAllSignalsTcalParams->GetAt(sig * 1000 + bin)
        << endl;
        }
      */
    }
