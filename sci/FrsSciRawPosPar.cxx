...
FrsSciRawPosPar::FrsSciRawPosPar(const char *name, const char *title,
                                 const char *context)
    : FairParGenericSet(name, title, context), fNumDetectors(2),
      fNumChannels(2), fNumParsPerSignal(2) {
...
void FrsSciRawPosPar::printParams() {
  ...
  for (Int_t param = 0; param < array_size; param++) {
    cout << "LIMIT " << param << " = " << fAllSignalsRawPosParams->GetAt(param)
         << endl;
  }
}
