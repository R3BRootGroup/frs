...
R3BWasaOnlineSpectra::R3BWasaOnlineSpectra()
    : FairTask("WasaOnlineSpectra", 1), fMappedItemsMdc(NULL), fTrigger(-1),
      fNEvents(0) {}

R3BWasaOnlineSpectra::R3BWasaOnlineSpectra(const char *name, Int_t iVerbose)
    : FairTask(name, iVerbose), fMappedItemsMdc(NULL), fTrigger(-1),
      fNEvents(0) {}
...
InitStatus R3BWasaOnlineSpectra::Init() {
  ...
  for (Int_t i = 0; i < 12 * 2; i++)
    mainfolWasa->Add(cMdcWasa[i]);

  run->AddObject(mainfolWasa);

  // Register command to reset histograms
  run->GetHttpServer()->RegisterCommand(
      "Reset_MDC", Form("/Objects/%s/->Reset_MDC_Histo()", GetName()));
