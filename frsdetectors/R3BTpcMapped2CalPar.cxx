...
InitStatus R3BTpcMapped2CalPar::Init() {
  ...
  fh_Map_xy[d] = new TH2F(name,name,fMapHistos_bins*2,-fMapHistos_right,fMapHistos_right,fMapHistos_bins,fMapHistos_left,fMapHistos_right);
  ...
  for(Int_t i = 0; i < nHits; i++) {
   ...
   //std::cout << fTpcrt->GetAt(0) <<" "<< fTpcrt->GetAt(1) << std::endl;
    ...
    for(int j=0;j<fNumAnodesY;j++){
      ...
      if(tpc_csum[j]>fLim_Csum_Min->GetAt(detId*fNumAnodesY+j) && tpc_csum[j]<fLim_Csum_Max->GetAt(detId*fNumAnodesY+j))
         b_tpc_csum[j] = kTRUE;
    ...
    if ((b_tpc_csum[0] || b_tpc_csum[1]) && (fTpclt[0]-fTpcrt[0])<fMaxX && fTpcdt[0]>1600 && fTpcdt[1]>1600){
    ...
    if ((b_tpc_csum[2] || b_tpc_csum[3]) && (fTpclt[1]-fTpcrt[1])<fMaxX && fTpcdt[2]>1600 && fTpcdt[3]>1600){
...
void R3BTpcMapped2CalPar::SearchMax(double *xpeaks, double *ypeaks, int nf, double *xpos){
  ...
  int p=0;
  for(int i=0;i<nf;i++){
   if(ypeaks[i]>max){p=i;max=ypeaks[i];}
  }
  x[k]=xpeaks[p];
  ypeaks[p]=0;
  }
