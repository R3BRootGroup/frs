...
void R3BMusicMapped2Cal::SetParContainers() {
  ...
  if (!fCal_Par) {
    ...
    LOG(INFO)<<"R3BMusicMapped2CalPar:: musicCalPar container open"<<FairLogger::endl;
  }
}
...
void R3BMusicMapped2Cal::SetParameter(){
  ...
  LOG(INFO)<<"R3BMusicMapped2Cal: Nb detectors: "<< NumDets <<FairLogger::endl;
  LOG(INFO)<<"R3BMusicMapped2Cal: Nb anodes: "<< NumAnodes <<FairLogger::endl;
  LOG(INFO)<<"R3BMusicMapped2Cal: Nb parameters from pedestal fit: "<< NumParams <<FairLogger::endl;
  ...
  LOG(INFO)<<"R3BMusicMapped2Cal: Nb of dead anodes in MUSIC " <<d+1<< ": "<< numdeadanodes <<FairLogger::endl;
  }
}
...
{
  LOG(INFO) << "R3BMusicMapped2Cal: Init" << FairLogger::endl;
  ...
  if (!fMusicMappedDataCA) { return kFATAL;}
...
void R3BMusicMapped2Cal::Exec(Option_t* option)
{
  ...
  if(nHits!=NumAnodes*NumDets && nHits>0)LOG(WARNING) << "R3BMusicMapped2Cal: nHits!=" <<nHits<<" NumAnodes:NumDets"<< NumAnodes << ":"<< NumDets <<FairLogger::endl;
  if(!nHits) return;
...
{
  LOG(DEBUG) << "Clearing MusicCalData Structure" << FairLogger::endl;
  if(fMusicCalDataCA)fMusicCalDataCA->Clear();
}
